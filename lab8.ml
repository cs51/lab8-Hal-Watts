...
let fakeNewsNetwork_id = WEvent.add_listener newswire fakeNewsNetwork ;;
...
WEvent.remove_listener newswire fakeNewsNetwork_id ;;
...
let receive_report = fun (s : string) -> ignore (WEvent.add_listener publish
                        (fun _ -> fakeNewsNetwork s; buzzFake s)) ;;
